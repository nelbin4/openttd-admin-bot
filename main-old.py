...
def build_server_configs(settings: Dict[str, Any]) -> List[Config]:
    ...
    for idx, admin_port in enumerate(admin_ports):
        ...
        cfg = Config(
            ...
            game_port=game_port,
            server_num=idx + 1,
            server_ip=str(required["server_ip"]),
            admin_name=str(required["admin_name"]),
            admin_pass=str(required["admin_pass"]),
            goal_value=int(required["goal_value"]),
            load_scenario=str(required["load_scenario"]),
            dead_co_age=int(required["dead_co_age"]),
            dead_co_value=int(required["dead_co_value"]),
            rcon_retry_max=int(required["rcon_retry_max"]),
            rcon_retry_delay=float(required["rcon_retry_delay"]),
            reconnect_max_attempts=int(required["reconnect_max_attempts"]),
            reconnect_delay=float(required["reconnect_delay"]),
            reset_countdown_seconds=int(required["reset_countdown_seconds"]),
        )
        configs.append(cfg)
    return configs
...
MONITOR_INTERVAL_WEEKLY = 18     # ~1 week in game time (2.5s per day)
MONITOR_INTERVAL_QUARTERLY = 300  # 5 minutes in game time (2.5s per day)
MONITOR_INTERVAL_MONTHLY = 120   # 2 minutes in game time (2.5s per day)
...
class RconHandler:
    ...
    def __init__(self, admin: 'SafeAdmin', logger: logging.Logger, config: Config):
        self.admin = admin
        self.logger = logger
        self.config = config
        self.lock = threading.RLock()
    ...
    def _validate_response(self, response: str, cmd: str) -> bool:
        if not response:
            return True
    ...
    def execute(self, cmd: str, timeout: Optional[float] = None, escape_args: bool = True) -> str:
        if escape_args:
        ...
        for attempt in range(self.config.rcon_retry_max):
            try:
                ...
                if attempt == self.config.rcon_retry_max - 1:
                    self.logger.warning("RCON failed after %d attempts: %s - %s",
                                        self.config.rcon_retry_max, cmd, e)
                    return ""
                time.sleep(self.config.rcon_retry_delay * (attempt + 1))
        return ""

    def _execute_once(self, cmd: str, timeout: float) -> str:
        ...
        with self.lock:
            while self.inflight:
                ...
                if remaining <= 1.0:
                    self.logger.warning("RCON wait nearing timeout; current_cmd=%s blocking new cmd=%s",
                                        self.current_cmd, cmd)
                if remaining <= 0:
                    self.logger.warning("RCON wait timeout; force-clearing inflight for cmd=%s (stuck on %s)",
    ...
    def _recv_main(self, deadline: float, cmd: str) -> str:
    ...
    def _recv_thread(self, deadline: float, cmd: str) -> str:
        with self.lock:
            while time.time() < deadline:
                if not self.inflight and self.end_ts:
                    if time.time() - self.end_ts >= RCON_GRACE:
    ...
    def on_rcon(self, text: str) -> None:
        if text:
            with self.lock:
                if self.inflight and len(self.buffer) < RCON_BUFFER_MAX:
                    self.buffer.append(text)
                    self.cv.notify_all()

    def on_rcon_end(self) -> None:
        with self.lock:
            if self.inflight:
...
class SafeAdmin:
    ...
    def connect(self, admin_name: str, admin_pass: str, logger: logging.Logger) -> bool:
        try:
    ...
    def ensure_connected(self, admin_name: str, admin_pass: str, logger: logging.Logger) -> bool:
...
class CommandHandler:
    def __init__(self, bot: 'OpenTTDBot'):
        self.bot = bot

    def execute(self, cid: int, args: List[str], state: Dict[str, Any]) -> None:
...
class ResetCommand(CommandHandler):
    def execute(self, cid: int, args: List[str], state: Dict[str, Any]) -> None:
        ...
        if co_id is None or co_id == SPECTATOR_ID:
        ...
        with self.bot.reset_lock:
            if cid in self.bot.reset_timers and self.bot.reset_timers[cid]:
...
class YesCommand(CommandHandler):
    def execute(self, cid: int, args: List[str], state: Dict[str, Any]) -> None:
        ...
        if current_co != pending_co_id:
            spectator_msg = 'spectator' if current_co == SPECTATOR_ID else f'company {current_co}'
            self.bot.send_msg(
                f"Reset cancelled: you were in company {pending_co_id} but are now in {spectator_msg}. "
...
class OpenTTDBot:
    def __init__(self, cfg: Config, total_servers: int = 1):
        self.cfg = cfg
        ...
        self.rcon: Optional[RconHandler] = None
        ...
        self.reset_timers: Dict[int, Optional[threading.Timer]] = {}

        self.paused_lock = threading.Lock()

        self.cleanup_lock = threading.Lock()
        self.cleanup_in_progress: Set[int] = set()
        self.cleanup_semaphore = threading.Semaphore(CLEANUP_SEMAPHORE_MAX)
        self.refresh_lock = threading.Lock()

        self.command_cooldown: Dict[int, float] = {}
        ...
        self.memory_tracker: Optional[bool] = None
        self.last_cleanup = time.time()
        self.reconnect_attempts = 0

        self.logger.info("Bot init: server=%s:%d (workers=%d)",
                         self.cfg.server_ip, self.cfg.admin_port, worker_count)

    def _validate_config(self) -> None:
        if not (1024 <= self.cfg.admin_port <= 65535):
            raise ValueError(f"Invalid admin_port: {self.cfg.admin_port}")
        if self.cfg.game_port and not (1024 <= self.cfg.game_port <= 65535):
    ...
    def _register_handlers(self) -> None:
        handlers = [
            (openttdpacket.ChatPacket, self.on_chat),
            (openttdpacket.ClientJoinPacket, self.on_join),
            (openttdpacket.ClientInfoPacket, self.on_client_info),
            (openttdpacket.ClientUpdatePacket, self.on_update),
            (openttdpacket.ClientQuitPacket, self.on_client_quit),
            (openttdpacket.CompanyInfoPacket, self.on_company_info),
            (openttdpacket.CompanyUpdatePacket, self.on_company_update),
            (openttdpacket.CompanyEconomyPacket, self.on_company_economy),
            (openttdpacket.CompanyRemovePacket, self.on_company_remove),
            (openttdpacket.RconPacket, self.on_rcon),
            (openttdpacket.RconEndPacket, self.on_rcon_end),
            (openttdpacket.ErrorPacket, self.on_error),
            (openttdpacket.NewGamePacket, self.on_new_game),
        ]
        for packet_type, handler in handlers:
            self.admin.add_handler(packet_type)(handler)
    ...
    def _subscribe_updates(self) -> None:
        subscriptions = [
            ...
            (AdminUpdateType.COMPANY_INFO, AdminUpdateFrequency.POLL),
            (AdminUpdateType.COMPANY_ECONOMY, AdminUpdateFrequency.WEEKLY),
            (AdminUpdateType.COMPANY_ECONOMY, AdminUpdateFrequency.POLL),
            (AdminUpdateType.COMPANY_STATS, AdminUpdateFrequency.WEEKLY),
            (AdminUpdateType.COMPANY_STATS, AdminUpdateFrequency.POLL),
        ]
        for update_type, frequency in subscriptions:
            if frequency:
                self.admin.subscribe(update_type, frequency)
            else:
                self.admin.subscribe(update_type)
    ...
    def _perform_periodic_cleanup(self) -> None:
        ...
        with self.reset_lock:
            expired_cids = [
                cid for cid, timer in self.reset_timers.items()
        ...
        with self.command_cooldown_lock:
            old_cooldowns = [
                cid for cid, ts in self.command_cooldown.items()
                if now - ts > 60
            ]
    ...
    def _check_command_cooldown(self, cid: int) -> bool:
        with self.command_cooldown_lock:
            now = time.time()
    ...
    def on_chat(self, admin, pkt) -> None:
    ...
    def on_join(self, admin, pkt) -> None:
        cid = pkt.id
    ...
    def on_client_info(self, admin, pkt) -> None:
        cid = pkt.id
        co_id = pkt.company_id + 1 if pkt.company_id not in (None, SPECTATOR_ID) else pkt.company_id
        self.clients_cache.set(cid, {'name': pkt.name, 'company': co_id})
        self.cache_ts = time.time()
        self._schedule_pause_check()

    def on_client_quit(self, admin, pkt) -> None:
        cid = pkt.id
        with self.reset_lock:
    ...
    def on_update(self, admin, pkt) -> None:
        cid = pkt.id
        co_id = pkt.company_id + 1 if pkt.company_id not in (None, SPECTATOR_ID) else pkt.company_id
        ...
        if cached.get('company') != co_id:
            self._cancel_reset_for_company_switch(cid)

        self._schedule_pause_check()

    def _cancel_reset_for_company_switch(self, cid: int) -> None:
        with self.reset_lock:
            if cid in self.reset_pending:
                self.reset_pending.pop(cid, None)
                timer = self.reset_timers.pop(cid, None)
    ...
    def on_new_game(self, admin, pkt) -> None:
    ...
    def on_rcon(self, admin, pkt) -> None:
        if self.rcon:
            self.rcon.on_rcon(pkt.response.strip())

    def on_rcon_end(self, admin, pkt) -> None:
        if self.rcon:
            self.rcon.on_rcon_end()

    def on_error(self, admin, pkt) -> None:
        current_cmd = self.rcon.current_cmd if self.rcon else None
        self.logger.warning("Server ErrorPacket: %s (current_cmd=%s)",
                            getattr(pkt, 'error', pkt), current_cmd)
    ...
    def _run_pause_check(self) -> None:
        with self.pause_check_lock:
            self.pause_check_scheduled = False
    ...
    def _refresh_game_state(self) -> Dict[str, Dict[int, Dict[str, Any]]]:
        companies = dict(self.companies_cache.items())
        clients = dict(self.clients_cache.items())

        if self._cache_valid():
            has_client_company = any(
                c.get('company') not in (None, SPECTATOR_ID)
        ...
        now = time.time()
        need_refresh = (
                (now - self.last_companies_rcon >= RCON_REFRESH_TTL) or
    ...
    def _batch_refresh_state(self, reason: str = "") -> None:
        if not USE_RCON_FALLBACK:
            return
        ...
        with self.refresh_lock:
            ...
            self.logger.debug("[Rcon] Using fallback for %s (reason=%s)",
                              ','.join(commands), reason or 'cache_refresh')
    ...
    def _parse_companies(self, output: str, reason: str = "") -> None:
        if not output.strip():
            return
        ...
        for line in output.splitlines():
            ...
            try:
                ...
                self.companies_cache.set(co_id, {
                    ...
                    'start_date': int(year),
                    'money': parse_number(money),
                    'loan': parse_number(loan),
        ...
        if updated:
            ...
            self.logger.warning(
                ...
                reason or "n/a", unmatched
            )

    def _parse_clients(self, output: str) -> None:
        if not output.strip():
            return
        ...
        for line in output.splitlines():
            ...
            try:
                ...
                company_id = parsed_co_int if parsed_co_int != SPECTATOR_ID else SPECTATOR_ID
    ...
    def _get_current_year(self, companies: Dict[int, Dict[str, Any]]) -> int:
        ...
        if out:
            ...
            if m:
                year_str = m.group(1)
                try:
                    parsed = datetime.strptime(year_str, "%Y-%m-%d").date()
                    return parsed.year
                except ValueError:
    ...
    def _update_companies_from_rcon(self, reason: str = "", force: bool = False) -> None:
        if not USE_RCON_FALLBACK:
            return
        ...
        with self.refresh_lock:
            ...
            has_data = bool(list(self.companies_cache.items()))

            should_refresh = force or (not has_data) or (age >= RCON_REFRESH_TTL)
            if not should_refresh:
                self.logger.debug(
                    "[Rcon] Skip companies refresh: age=%.2fs < ttl=%.2fs (reason=%s)",
                    age, RCON_REFRESH_TTL, reason or "n/a"
                )
                return

            if reason:
                self.logger.debug("[Rcon] Using fallback for companies (reason=%s, force=%s)",
                                  reason, force)
            output = self.rcon.execute('companies')
            self._parse_companies(output, reason)

    def _update_clients_from_rcon(self, reason: str = "", force: bool = False) -> None:
        if not USE_RCON_FALLBACK:
            return
        ...
        with self.refresh_lock:
            ...
            has_data = bool(list(self.clients_cache.items()))
            ...
            if reason:
                self.logger.debug("[Rcon] Using fallback for clients (reason=%s, force=%s)",
                                  reason, force)
    ...
    def _company_refresh_loop(self) -> None:
        while not self.stop_event.wait(60):
    ...
    def _cleanup_unnamed_company(self) -> bool:
        try:
    ...
    def _get_active_companies(self, companies: Dict[int, Dict[str, Any]],
                              clients: Dict[int, Dict[str, Any]]) -> Dict[int, Dict[str, Any]]:
        active = {}
    ...
    def _check_pause_state(self) -> None:
    ...
    def _monitor_tick(self) -> float:
        try:
            ...
            # Move to ACTIVE once companies are present so goal transitions can fire
            with self.phase_lock:
                if active_companies and self.phase == GamePhase.WAITING:
            ...
            # For goal checking, use packet data directly to avoid RCON fallbacks
            # Use the live companies cache populated from packet subscriptions
            packet_companies = dict(self.companies_cache.items()) or active_companies
            self.logger.debug("Using companies cache for goal check: %d companies", len(packet_companies))

            interval = self._check_goal(packet_companies)
    ...
    def send_msg(self, msg: str, cid: Optional[int] = None) -> None:
    ...
    def _check_goal(self, companies: Dict[int, Dict[str, Any]]) -> float:
        if not companies:
            return MONITOR_INTERVAL_NO_CO

        # Use packet data directly - no RCON fallbacks. Source: companies_cache
        packet_companies = dict(self.companies_cache.items()) or companies
        self.logger.debug("Goal check using companies cache: %d companies", len(packet_companies))

        if not packet_companies:
            return MONITOR_INTERVAL_NO_CO

        # Find top company by value
        top_id, top = max(packet_companies.items(), key=lambda x: x[1]['value'])
        ratio = top['value'] / self.cfg.goal_value

        # Pure packet-based goal checking - no RCON calls
        ...
        # Adaptive monitoring frequency based on goal progress
        if ratio >= 0.95:
            # Company is very close to goal - use weekly monitoring
            self.logger.debug("Company %s at %.1f%% of goal, using weekly monitoring", 
                             top['name'], ratio * 100)
            return MONITOR_INTERVAL_WEEKLY
        else:
            # Default to quarterly monitoring
            self.logger.debug("Company %s at %.1f%% of goal, using quarterly monitoring", 
                             top['name'], ratio * 100)
            return MONITOR_INTERVAL_QUARTERLY

    def _check_dead_companies(self, companies: Dict[int, Dict[str, Any]],
                              clients: Dict[int, Dict[str, Any]]) -> None:
        if not companies:
            return
        ...
        futures = [
            self.executor.submit(self._cleanup_company, co_id, co_data)
    ...
    def _cleanup_company(self, co_id: int, co_data: Dict[str, Any]) -> None:
        with self.cleanup_lock:
            if co_id in self.cleanup_in_progress:
    ...
    def _handle_goal(self, winner: str, value: int) -> None:
        try:
            with self.phase_lock:
            ...
            msg = (f"=== GOAL ACHIEVED ===\n"
                   ...
                   f"Map restart in {self.cfg.reset_countdown_seconds}s...")
            self.send_msg(msg)
    ...
    def _transition_phase(self, new_phase: GamePhase) -> bool:
    ...
    def _get_client_company_rcon(self, cid: int) -> Optional[int]:
        ...
        for line in out.splitlines():
            ...
            try:
                ...
                if int(parsed_cid) == cid:
                    parsed_co_int = int(parsed_co)
                    return parsed_co_int if parsed_co_int != SPECTATOR_ID else SPECTATOR_ID
            except (ValueError, AttributeError):
                continue
        self.logger.warning("[Rcon] cid=%s not found in clients output", cid)
        return None

    def _company_exists_rcon(self, co_id: int) -> bool:
    ...
    def _process_cmd(self, cid: int, msg: str) -> None:
        parts = msg[1:].split()
        if not parts:
            return
    ...
    def _cancel_reset(self, cid: int) -> None:
        with self.reset_lock:
            self.reset_pending.pop(cid, None)
            timer = self.reset_timers.pop(cid, None)
            if timer:
