# OpenTTD Admin Bot
  ...
  [![Python](https://img.shields.io/badge/python-3.10+-blue?logo=python&logoColor=white)](h
  ttps://www.python.org/)
  [![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
  [![OpenTTD](https://img.shields.io/badge/OpenTTD-14.0+-green.svg)](https://www.openttd.or
  g/)

  Async Python bot for managing OpenTTD multiplayer servers. Handles auto-pause, goal
  tracking, company cleanup, and player engagement. Supports multiple servers from a single
   instance.
  ...
  - 60s poll: rcon companies runs at every wall-clock :00 second; updates value cache,
  triggers goal check and auto-clean
  - Auto-clean: resets companies where age ≥ clean_age AND value < clean_value; moves
  clients to spectator first
  - Goal: when any company reaches goal, announces winner, counts down 20s, reloads map
  - Limit enforcement: if a client already owns max_companies companies, any extra company
  ...
  - Map not loading: verify file exists in OpenTTD save/ or scenario/; use filename only
  (e.g. map.scn)
  ...
  **Changes from your original:**
  - Removed `!help` from commands table (the bot sends `!info !rules !cv !reset` as the
  help response — there's no separate `!help` handler)
  - Removed `MAX_COMPANIES_PER_IP` reference → `max_companies` (matches actual config key)
  - Updated example `goal`/`clean_age`/`clean_value` to match what's actually running (100b
   goal, 5yr/5m thresholds)
  - Removed the commented-out `[server2]` block — one clean example is enough, the note
  covers it
  - Removed `(from 60s cache)` from `!cv` — it's live data from the in-memory cache, not a
  separate cache TTL
